...
knitr::opts_chunk$set(
  ...
  warning = FALSE,
  message = FALSE,
  fig.path = "./results/05-WGCNA/Analysis_figure/"
)
...
condition_list <- condition_list %>% filter(
  group %in% c("CTRL_Vehicle", "22q_Vehicle")
)
...
DEG_1.0 <- read.csv("results/02-DEG-Vehicle/02-DEG/02_DEG_log2fc_1.csv")
...
cairo_pdf(
  filename = file.path("results", "05-WGCNA", "1.dendrogram.pdf"),
  width    = 15,
  height   = 8,
  bg       = "transparent"
)
...
png(
  filename = file.path("results", "05-WGCNA", "1.dendrogram.png"),
  width    = 15,
  height   = 8,
  units    = "in",       # inches
  res      = 300,        # 300 DPI
...
write.csv(module_genes, file = "results/05-WGCNA/module_genes.csv", row.names = F)
...
trait <- ifelse(condition_list$group == "22q_Vehicle", 1, 0)
...
write.csv(corr_df2, file = "results/05-WGCNA/module_trait_correlation.csv", row.names = F)
```
...
traitNum <- ifelse(condition_list$group == "22q_Vehicle", 1, 0)
...
#––––– Transparent PDF –––––#
cairo_pdf(
  filename = file.path("results", "05-WGCNA", "2.heatmap.pdf"),
  width    = 6,
  height   = 12,
  bg       = "transparent",
  family   = "Arial"
...
#––––– Transparent PNG –––––#
png(
  filename = file.path("results", "05-WGCNA", "2.heatmap.png"),
  width    = 6,
  height   = 12,
  units    = "in",
...
# base_dir <- "results/05-WGCNA/module_result"
# 
# # Get list of modules
