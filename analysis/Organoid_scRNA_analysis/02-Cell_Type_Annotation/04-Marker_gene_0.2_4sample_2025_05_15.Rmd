...
knitr::opts_chunk$set(
  ...
  fig.width = 9,
  fig.height = 4,
  fig.path = "result/04-Cluster-Marker-4sample/Analysis_plot_res_0.2/"
)
...
# delect the C3 and Q3 from the data
...
FindAllMarkers(seurat_obj_marker, assay = "RNA", only.pos = T, densify = T) -> cluster_markers
...
dir.create(sprintf("./result/04-Cluster-Marker-4sample/res_%s", res), showWarnings = FALSE, recursive = TRUE)
...
lapply(unique(cluster_marker$cluster), function(cl){
  ...
  openxlsx::write.xlsx(
    cell.type.annotation,
    file = sprintf("./result/04-Cluster-Marker-4sample/res_%s/cell_type_annotation_resolution_%s.xlsx", res,res)
  )
...
write.csv(unique_markers, sprintf("result/04-Cluster-Marker-4sample/res_%s/unique_markers.csv", res), row.names = FALSE)
...
for (i in  1:nrow(markers)) {
  ...
  # plot the gene
  file_path <- sprintf("result/04-Cluster-Marker-4sample/res_%s/Gene_%s", res, gene)
...
gene_list_raw <- "PAX6,MKI67,TOP2A"
